...
use pb::substreams::v1::program::PoolMediaSetEvent;

use sologger_log_context::programs_selector::ProgramsSelector;
use sologger_log_context::sologger_log_context::LogContext;
use substreams_solana::pb::sf::solana::r#type::v1::Block;

const PROGRAM_ID: &str = "6qXrYhcXYzKDCU6XoSJY94oNGF4QZ31KGbmUTDW8YQPP";

#[substreams::handlers::map]
fn map_program_data(blk: Block) -> Data {
    let mut pool_created_event_list: Vec<PoolCreatedEvent> = Vec::new();
    let mut create_pool_instruction_list: Vec<CreatePoolInstruction> = vec![];
    let mut initialize_instruction_list: Vec<InitializeInstruction> = vec![];
    let mut bet_placed_event_list: Vec<BetPlacedEvent> = vec![];
    let mut place_bet_instruction_list: Vec<PlaceBetInstruction> = vec![];
    let mut pool_media_set_event_list: Vec<PoolMediaSetEvent> = vec![];
                ...
                context.data_logs.iter().for_each(|data| {
                    if let Ok(decoded) = BASE64_STANDARD.decode(data) {
                        ...
                        match static_discriminator_slice {
                            idl::idl::program::events::PoolCreated::DISCRIMINATOR => {
                                    ...
                                    )
                                {
                                    pool_created_event_list.push(PoolCreatedEvent {
                                        ...
                                        media_type: map_enum_media_type(event.media_type),
                            ...
                            idl::idl::program::events::PoolMediaSet::DISCRIMINATOR => {
                                if let Ok(event) =
                                    idl::idl::program::events::PoolMediaSet::deserialize(
                                        &mut &slice_u8[8..],
                                    )
                                {
                                    pool_media_set_event_list.push(PoolMediaSetEvent {
                                        tx_hash: transaction.id(),
                                        pool_id: event.pool_id,
                                        media_url: event.media_url,
                                        media_type: map_enum_media_type(event.media_type),
                ...
                if &slice_u8[0..8] == idl::idl::program::client::args::CreatePool::DISCRIMINATOR {
                        ...
                        )
                    {
                        ...
                        create_pool_instruction_list.push(CreatePoolInstruction {
                            ...
                            media_type: map_enum_media_type(instruction.media_type),
...
fn map_enum_token_type(token_type: idl::idl::program::types::TokenType) -> i32 {
    match token_type {
        idl::idl::program::types::TokenType::Usdc { .. } => 0,
        idl::idl::program::types::TokenType::Points { .. } => 1,
    }
}

fn map_enum_media_type(media_type: idl::idl::program::types::MediaType) -> i32 {
    match media_type {
        idl::idl::program::types::MediaType::X { .. } => 0,
        idl::idl::program::types::MediaType::TikTok { .. } => 1,
        idl::idl::program::types::MediaType::Instagram { .. } => 2,
        idl::idl::program::types::MediaType::Facebook { .. } => 3,
        idl::idl::program::types::MediaType::Image { .. } => 4,
        idl::idl::program::types::MediaType::Video { .. } => 5,
        idl::idl::program::types::MediaType::ExternalLink { .. } => 6,
    }
}
